 ...
 */
// 1
var lefthandWeapon: String?  = "Knife"
...
if let leftWeapon = lefthandWeapon, let rightWeapon = righthandWeapon {
    ...
    print("i didnt bring enough hardware for this...")
}
// 3
let playerExp = ["Kill": 20, "Dead": -20, "Objective Complete": 100]
// 4
for (incident, experience) in playerExp {
    print("\(incident): \(experience)")
}
// 5
for (incident, experience) in playerExp {
    guard experience < 0 else {
        print("\(incident) has higher xp than 0")
        continue
    }
    print("\(incident) has lower xp than 0")
}
// 6
var exp = 200
...
case (200...500):
    print("exp is greater than 199 and lesser than 501")
case (let localExp) where localExp > 500:
    print("exp is greater than 500")
default:
    print("exp is not recognized")
}
